...
def main():
    ap = argparse.ArgumentParser(
        description="Create a new analysis run directory and write an analysis_config.json template."
    )
    ap.add_argument("--root", required=True, help="Root directory where run directories are created")
    ap.add_argument(
        "--model-id",
        required=True,
        help="Stable model identifier used under the run root",
    )
    ap.add_argument(
        "--run-name",
        required=True,
        help="Human-readable run name within the model directory",
    )
    ap.add_argument(
        "--model-path",
        required=False,
        default=None,
        help="Optional initial model path written into analysis_config.json",
    )
    args = ap.parse_args()
