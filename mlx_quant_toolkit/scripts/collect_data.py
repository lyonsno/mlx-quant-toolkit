...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Scan model files, extract expert matrices, compute stats, and optionally run "
            "MLX quantization simulation."
        )
    )
    ap.add_argument("--run-dir", required=True, help="Run directory created by init_run")
    ap.add_argument(
        "--model-path",
        default=None,
        help="Override the model_path from analysis_config.json",
    )
    args = ap.parse_args()
